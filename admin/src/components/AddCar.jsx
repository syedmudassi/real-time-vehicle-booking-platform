  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const fieldMappings = {
        ...
        // gpsEnabled: String(Boolean(data.gpsEnabled)),
      ...
      // GPS system disabled
      // const trackerId = response?.data?.gpsVehicleId;
      // const trackerMsg = trackerId
      //   ? ` Tracker ID: ${trackerId} (copy this into the Android app).`
      //   : "";
            ...
            <div className={AddCarPageStyles.formColumn}>
              ...
              <div className="rounded-xl border border-gray-200/60 bg-gray-50 p-4">
                ...
                <p className="mt-1 text-xs text-gray-400">
                  Enable GPS to use an Android phone as tracker. Leave Tracker ID blank
                  and the backend auto-generates one (e.g. TRK-A1B2C3D4). Copy it into
                  the Android app after saving.
                </p>
                ...
                <label className="mt-3 flex items-center gap-2 text-sm text-gray-700">
                  <input
                    type="checkbox"
                    name="gpsEnabled"
                    checked={Boolean(data.gpsEnabled)}
                    onChange={handleCheckbox}
                  />
                  Enable GPS tracking for this car
                </label>

                <div className="mt-3">
                  <label className={AddCarPageStyles.label}>
                    Vehicle Tracker ID (optional)
                  </label>
                  <input
                    name="gpsVehicleId"
                    value={data.gpsVehicleId}
                    onChange={handleChange}
                    type="text"
                    className={AddCarPageStyles.input}
                    placeholder="e.g. VEH-001 (or leave blank)"
                    disabled={!data.gpsEnabled}
                  />
                </div>
              </div>
              */}
