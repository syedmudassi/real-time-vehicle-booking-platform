...
const CHAT_SUMMARY_ENDPOINT = "/api/chats/my/summary";
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    ...(isLoggedIn ? [{ to: "/profile", label: "My Profile" }] : []),
  ];
  ...
  const loadChatSummary = useCallback(async () => {
    ...
    try {
      const res = await api.get(CHAT_SUMMARY_ENDPOINT, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const unread = Number(res?.data?.summary?.unreadForUser || 0);
  ...
  useEffect(() => {
    loadChatSummary();
  }, [loadChatSummary, location.pathname]);
  ...
  useEffect(() => {
    ...
    const socket = io(base, {
      ...
      auth: { role: "user", token },
    });
    socketRef.current = socket;

    socket.on("chat:thread-updated", (payload) => {
      const unread = Number(payload?.summary?.unreadForUser ?? payload?.thread?.unreadForUser ?? 0);
      setChatUnreadCount(unread);
    });
  ...
  return (
    <nav
      className={`${styles.nav.base} ${
        isHome ? styles.nav.homeBackdrop : ""
      } ${
