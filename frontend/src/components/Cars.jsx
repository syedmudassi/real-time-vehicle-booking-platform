...
const Cars = () => {
  ...
  const filteredCars = useMemo(
    ...
    [cars, search, category, transmission, priceBucket],
  );
  ...
  const formatDate = (dateStr) => {
    ...
    try {
      ...
      return new Intl.DateTimeFormat("en-PK", opts).format(d);
    } catch {
      return dateStr;
    }
  };
  //sIMILAR TO HOMECAR.JSX TO FTECH DATE AS PAKISTAN STANDARD
  ...
  const computeEffectiveAvailability = (car) => {
    ...
    if (Array.isArray(car.bookings) && car.bookings.length) {
      const overlapping = car.bookings
        ...
        .filter(Boolean)
        .filter((b) => {
          const status = String(b.raw?.status || "").toLowerCase();
          const paymentStatus = String(b.raw?.paymentStatus || "").toLowerCase();

          // Ignore any unpaid / pending bookings so availability behaves properly.
          if (status === "pending" || paymentStatus === "pending") return false;
  ...
  return (
    <div className={carPageStyles.pageContainer}>
      <div
        className={carPageStyles.headerContainer}
        style={{
          backgroundImage: `url("https://thumbs.dreamstime.com/b/close-up-modern-car-dashboard-night-advanced-tech-features-illuminated-displays-control-panels-luxury-interior-design-high-359006705.jpg")`,
          backgroundSize: "cover",
          backgroundRepeat: "no-repeat",
          backgroundPosition: "center top",
        }}
      >
        <div className={carPageStyles.headerOverlay} aria-hidden />
        <div className={carPageStyles.headerContent}>
          ...
          {!loading &&
            ...
            filteredCars.length === 0 && (
              <div className="col-span-full text-center text-gray-500">
                No cars match your filters. Try adjusting search or filters.
              </div>
            )}
                ...
                <div key={id} className={carPageStyles.carCard}>
                  ...
                  <div className={carPageStyles.imageContainer}>
                    ...
                    <div className={carPageStyles.priceBadge}>
                      Rs{car.dailyRate ?? car.price ?? car.pricePerDay ?? "—"}
                      /day
                    </div>
                  </div>
